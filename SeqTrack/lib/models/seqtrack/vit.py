...
try:
    ...
    print("timm not available, using fallback values")
    # Fallback values
    ...
    def trunc_normal_(tensor, mean=0., std=1., a=-2., b=2.):
        # Simple truncation without proper implementation
        with torch.no_grad():
            tensor.normal_(mean, std)
            tensor.clamp_(a, b)
    
    def DropPath(x, drop_prob=0.):
        if drop_prob == 0. or not TIMM_AVAILABLE:
            return x
